...
func (lv *LogView) WndProc(hwnd win.HWND, msg uint32, wParam, lParam uintptr) uintptr {
	// wp := fmt.Sprintf("%#x", wParam)
	// dbl := fmt.Sprint(DBT_DEVICEARRIVAL)
	// dbe := fmt.Sprint(DBT_DEVICEREMOVECOMPLETE)
	// if dbl == wp || dbe == wp {
	// 	fmt.Println("USB -> ")
	// }
	// 	fmt.Printf("wparam - >%#x\n", wParam)
	...
	case win.WM_DEVICECHANGE:
		if (wParam == winC.DBT_DEVICEARRIVAL) || (wParam == winC.DBT_DEVICEREMOVECOMPLETE) {
			fmt.Println(NotificationFilter.Dbcc_name)
