...
<body>
  <header id="header">
    <div class="github-widget" data-repo="CodyGuo/xcgui" style="margin:5px;">
    </div>
    <br><br>
    ...
    <div class="container">
      <div class="col-lg-4 col-lg-offset-4 col-sm-6 col-sm-offset-3 col-xs-8 col-xs-offset-2">
        <form class="form-inline" id="ping-Addr">
          <label style="padding: 10px">IP地址: </label>
          <div class="form-group">
            <input type="text" class="form-control" placeholder="请输入IP" id="ipAddr">
          </div>
          <div class="form-group">
            <button class="btn btn-primary" id="ping" type="button">Ping</button>
          </div>

          <div id="output" style="width: 130%">
              <pre id="log"></pre>
          </div>
      </div>
      </form>
  ...
  <script  type="text/javascript">
    ...
    function wsOpen(ip) {
        var conn = new WebSocket("ws://{{.Host}}/ws/log?ip="+ip);
        conn.onopen = function(evt){
          printDbg("WebSocket connected.")
        }
        conn.onclose = function(evt) {
          printDbg("WebSocket connection closed.");
          $(pingBtn).text("Ping").attr("data-type", "run").addClass("btn btn-primary").removeClass("btn-danger");
        }
    ...
    $(function(){
      $(pingBtn).click(function(){
        ...
        printDbg("开始ping..."+ip);
        $(pingBtn).text("Stop").attr("data-type", "stop").addClass("btn-danger").removeClass("btn-primary");
        $(log).empty()
